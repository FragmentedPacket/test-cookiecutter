...
NAME = os.getenv("IMAGE_NAME", "my-plugin")
PWD = os.getcwd()

BUILD_NAME = "my_plugin"
...
def destroy(context):
    ...
    docker_compose(context, "down")
    context.run(f"docker volume rm -f {BUILD_NAME}_pgdata_my_plugin")
...
def unittest(context, keepdb=False, verbosity=1):
    ...
    entrypoint = f"nautobot-server test my_plugin --verbosity={verbosity}"
    if keepdb:
        entrypoint += " --keepdb"
    command = f"run --entrypoint '{entrypoint}' nautobot"
...
def pylint(context):
    ...
    entrypoint = 'pylint --init-hook "import nautobot; nautobot.setup()" --rcfile /source/pyproject.toml /source/my_plugin'
    command = f"run --entrypoint '{entrypoint}' nautobot"
    docker_compose(
